...
* `ORKA_VM_USERDATA`: Specifies a userdata bootstrap script executed as root inside each runner VM on its first boot, before the GitHub runner starts. Use it to prepare the VM (install tools, enroll in MDM, register services) without baking the steps into the VM image. The script runs in the background and never delays or fails the VM boot; its output is written to `/var/log/orka/userdata.log` inside the VM. The script content is kept out of the integration's logs. Requires Orka 3.7.0 or later (both the cluster and the `orka3` CLI) and an Apple Silicon VM.
* `ORKA_VM_USERDATA_FILE_PATH`: Specifies a path to a file containing the userdata bootstrap script. Ignored when `ORKA_VM_USERDATA` is set.
